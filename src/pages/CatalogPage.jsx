...
function CatalogPage({bikes, loading, currency, setCurrency, cart, setCart}){
   ...
   function addCart(id, price) {
    ...
    return (
        <>
            <CatalogHeader searchBikes={searchBikes} cart={cart}/>
                    ...
                    { !loading ? sortedBikes.length > 0 ? sortedBikes.slice(0, amountBike).map((bike) => {
                        return (
                            <div className={sortedBikes.length <= 4 ? 'bike-card-container only-one' : 'bike-card-container'} key={bike.id}>
                                ...
                                <div className={ bike.status != 'available' ? 'cart-container not-allowed' : !cart.find(cartItem => cartItem.id == bike.id) ? 'cart-container' :  'cart-container on-cart'}
                                    onClick={() => {
                                        if(bike.status == 'available'){
                                ...
                                >
                                    ...
                                    {!cart.find(cartItem => cartItem.id == bike.id)? pickIconBike(bike.status) : <CheckIcon/>}
                                </div>
                            </div>
                        )
